...
// Δομή δεδομένων για τα arguments κάθε thread
struct ThreadArgs {
    int graph_counter;
    const std::set<int>* f;
    std::vector<std::vector<float>>* nodes_cords;
    const std::vector<std::vector<int>>* p_indexes;
    int R;
    float a;
    int l;
    std::map<int, int>* medoidsMap;
    std::vector<Graph>* graphs;
    std::string folderName;
    std::mutex* medoidsMapMutex;
};

// Συνάρτηση για το thread
void* processGraph(void* args_void) {
    ThreadArgs* args = static_cast<ThreadArgs*>(args_void);

    int graph_counter= args->graph_counter;
    const std::set<int>& f = *(args->f);
    std::vector<std::vector<float>>& nodes_cords = *(args->nodes_cords);
    const std::vector<std::vector<int>>& p_indexes = *(args->p_indexes);
    int R = args->R;
    float a = args->a;
    int l = args->l;
    std::map<int, int>& medoidsMap = *(args->medoidsMap);
    std::vector<Graph>& graphs = *(args->graphs);
    std::string folderName = args->folderName;
    string filename = folderName + "/graph_" + to_string(graph_counter) + ".bin";
        ...
        uniform_int_distribution<> dis(0, Fcordinates.size() - 1); // Τυχαίοι αριθμοί στο διάστημα [0, data.size() - 1]
        int medoid = dis(eng);
...
int main(int argc, char **argv) {
  ...
  for(int i=0; i < nodes.size(); i++){
    ...
    int graph_counter = 0;
    string folderName = "UpdatedOutGraphs";
    map<int, int> medoidsMap;
    // Κύριος κώδικας
    std::vector<pthread_t> threads(P.size());
    std::vector<ThreadArgs> threadArgs(P.size());
    std::mutex medoidsMapMutex;
    cout<<"Starting UpdatedStitchedVamana"<<endl;
    for(auto f: P){
        threadArgs[graph_counter] = {
        graph_counter,
        &P[graph_counter],
        &nodes_cords,
        &p_indexes,
        ...
        // Δημιουργία thread
        pthread_create(&threads[graph_counter], nullptr, processGraph, &threadArgs[graph_counter]);
        graph_counter++;  
    }   
    // Αναμονή για την ολοκλήρωση όλων των threads
    for (int graph_id = 0; graph_id < graph_counter; graph_id++) {
        pthread_join(threads[graph_id], nullptr);
    }
    cout<<"UpdatedStitchedVamana Finished!"<<endl;
